...
func (m *ProductMutation) SetDateUpdated(t time.Time) {
	m.dateUpdated = &t
}

// DateUpdated returns the value of the "dateUpdated" field in the mutation.
func (m *ProductMutation) DateUpdated() (r time.Time, exists bool) {
	v := m.dateUpdated
	if v == nil {
		return
	}
...
// If the Product object wasn't provided to the builder, the object is fetched from the database.
// An error is returned if the mutation operation is not UpdateOne, or the database query fails.
func (m *ProductMutation) OldDateUpdated(ctx context.Context) (v time.Time, err error) {
	if !m.op.Is(OpUpdateOne) {
